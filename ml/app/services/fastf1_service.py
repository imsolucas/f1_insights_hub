...
def fetch_current_season_drivers(year: int = CURRENT_SEASON) -> list[dict]:
    ...
    try:
        ...
        # Get first race session to fetch driver list
        ...
        for driver_code in drivers:
            try:
                ...
                driver_data = {
                    ...
                    "code": driver_code,
                    "forename": driver_info["FirstName"],
                    "surname": driver_info["LastName"],
                    "nationality": driver_info["CountryCode"],
                    "permanent_number": driver_info.get("DriverNumber"),
                    "current_team": team_name,
                    "is_active": True,
                    "driver_championships": get_driver_championships(driver_info["LastName"]),
                    "constructor_championships": get_constructor_championships(
                        driver_info["LastName"], team_name
                    ),
                }
                
                drivers_data.append(driver_data)
            except Exception as e:
                logger.error(f"Error fetching driver {driver_code}: {e}")
                continue
